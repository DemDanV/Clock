...
public class AnalogClockController : MonoBehaviour, ITimeDisplayer
{
    ...
    public void DisplayTime(DateTime time)
    {
        ...
        float seconds = time.Second + time.Millisecond / 1000f;  // Добавляем плавность через миллисекунды
    ...
    private void Update()
    {
        ...
        {
            float seconds = Time.deltaTime;  // Добавляем плавность через миллисекунды
            SecondArrow.transform.rotation *= Quaternion.Euler(0, 0, -seconds * 6f);
        }
    }
